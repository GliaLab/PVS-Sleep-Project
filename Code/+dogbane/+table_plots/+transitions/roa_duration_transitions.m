function roa_duration_transitions(tbl_roa_transitions)
...
mu = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_duration_transition,G);
sigma = splitapply(@(x)nanstd(x,1)/sqrt(size(x,1)), ...
    tbl_roa_transitions.roa_duration_transition,G);

mu_strict = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_duration_transition_strict,G);
sigma_strict = splitapply(@(x)nanstd(x,1)/sqrt(size(x,1)), ...
    tbl_roa_transitions.roa_duration_transition_strict,G);
%% Plot
output_folder = '~/Desktop/sleep_project/transitions/roa_duration_transitions';
...
for i = 1:size(mu,1)
    ...
    title(sprintf('ROA duration : %s : Start of %s',genotype(i),state(i)),'Interpreter','none');
    ylabel('Mean ROA duration (s)')

    % Save
    file_name = sprintf('roa_duration_%s_%s.png',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    file_name = sprintf('roa_duration_%s_%s.fig',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    ...
    % Save
    file_name = sprintf('roa_duration_%s_%s_strict.png',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    file_name = sprintf('roa_duration_%s_%s_strict.fig',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
