function roa_size_transitions(tbl_roa_transitions)
...
mu = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_size_transition,G);
sigma = splitapply(@(x)begonia.util.nansem(x,1), ...
    tbl_roa_transitions.roa_size_transition,G);

mu_strict = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_size_transition_strict,G);
sigma_strict = splitapply(@(x)begonia.util.nansem(x,1), ...
    tbl_roa_transitions.roa_size_transition_strict,G);
%% Plot
output_folder = '~/Desktop/sleep_project/transitions/roa_size_transitions';
...
for i = 1:size(mu,1)
    ...
    title(sprintf('ROA size : %s : Start of %s',genotype(i),state(i)),'Interpreter','none');
    ylabel('Mean ROA size (um^2)')

    % Save
    file_name = sprintf('roa_size_%s_%s.png',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    file_name = sprintf('roa_size_%s_%s.fig',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    ...
    % Save
    file_name = sprintf('roa_size_%s_%s_strict.png',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
    file_name = sprintf('roa_size_%s_%s_strict.fig',genotype(i),state(i));
    export_fig(f,fullfile(output_folder,file_name));
