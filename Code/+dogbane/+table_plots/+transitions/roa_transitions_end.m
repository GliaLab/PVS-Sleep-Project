function roa_transitions_end(tbl_roa_transitions)
...
mu = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_transition_end,G);
sigma = splitapply(@(x)begonia.util.nansem(x,1), ...
    tbl_roa_transitions.roa_transition_end,G);

mu_strict = splitapply(@(x)nanmean(x,1), ...
    tbl_roa_transitions.roa_transition_strict_end,G);
...
%% Plot
output_folder = '~/Desktop/sleep_project/transitions/roa_transitions_end';
...
for i = 1:size(mu,1)
    ...
    title(sprintf('ROA frequency : %s : End of %s',genotype(i),state(i)),'Interpreter','none');
