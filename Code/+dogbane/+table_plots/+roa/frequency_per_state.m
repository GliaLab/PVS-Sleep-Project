...
output_folder = '~/Desktop/sleep_project/bar_plots/roa_frequency_per_state';
...
response        = 'highpass_thresh_roa_freq';
predictor       = 'state';
model_formula   = sprintf('%s ~ %s  + (1 | dx_squared) + (-1 + state | mouse)',response,predictor);

tbl.highpass_thresh_roa_freq = tbl.highpass_thresh_roa_freq * 60 * 100;
