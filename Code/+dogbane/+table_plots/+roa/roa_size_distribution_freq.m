function roa_size_distribution_freq(table_roa_distributions)
...
roa_sizes_N_norm = splitapply(@(N,w) sum(N .* w,1) ./ sum(w), ...
    table_roa_distributions.roa_sizes_N_norm, ...
    table_roa_distributions.state_duration .* table_roa_distributions.fov_area, ...
    G);
roa_sizes_bin_edges = splitapply(@(x) x(1,:), ...
    table_roa_distributions.roa_sizes_bin_edges, ...
    G);
table_roa_distributions = table(genotype,state,roa_sizes_N_norm,roa_sizes_bin_edges);
%%
...
% Plot wt_dual
cla
title(sprintf('ROA Frequency by Size : wt_dual'),'interpreter','none');
hold on
I = find(table_roa_distributions.genotype == 'wt_dual')';
for i = I
    x = table_roa_distributions.roa_sizes_bin_edges(i,2:end);
    y = table_roa_distributions.roa_sizes_N_norm(i,:) * 100 * 60;
...
file_name = fullfile(output_folder,'roa_size_distribution_wt_dual.png');
export_fig(file_name);
file_name = fullfile(output_folder,'roa_size_distribution_wt_dual.fig');
export_fig(file_name);

% Plot ip3_dual
cla
title(sprintf('ROA Frequency by Size : ip3_dual'),'interpreter','none');
hold on
...
file_name = fullfile(output_folder,'roa_size_distribution_ip3_dual.png');
export_fig(file_name);
file_name = fullfile(output_folder,'roa_size_distribution_ip3_dual.fig');
export_fig(file_name);
...
for state = states
    ...
    cla
    title(sprintf('ROA Frequency by Size : %s',state),'interpreter','none');
    hold on
    ...
    file_name = sprintf('roa_size_distribution_%s.png',state);
    file_name = fullfile(output_folder,file_name);
    export_fig(file_name);
    file_name = sprintf('roa_size_distribution_%s.fig',state);
    file_name = fullfile(output_folder,file_name);
    export_fig(file_name);
end
end
