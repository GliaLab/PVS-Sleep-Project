function roi_response_per_roi(tbl,roi_group)
output_folder = '~/Desktop/bar_plots/roi_response_per_roi';

base_filename   = sprintf('roi_response_%s',roi_group);
plot_title      = sprintf('ROI average df/f0 in %s',roi_group);
plot_y_label    = 'df/f0';
response        = 'avg_response';
predictor_1     = 'genotype';
predictor_2     = 'state';
model_formula   = sprintf('%s ~ %s*%s + (-1 + state | mouse)',response,predictor_1,predictor_2);

tbl.genotype = setcats(tbl.genotype,{'wt_dual','ip3_dual'});
tbl(isundefined(tbl.genotype),:) = [];

tbl.state = setcats(tbl.state,{'locomotion','whisking','quiet','nrem','is','rem'});
tbl(isundefined(tbl.state),:) = [];

tbl.roi_group = setcats(tbl.roi_group,{roi_group});
tbl(isundefined(tbl.roi_group),:) = [];

tbl.avg_response = tbl.avg_response + 1;
