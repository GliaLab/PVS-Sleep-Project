function roa_in_endfeet_transitions_strict(trial)
ts = trial.tseries;
...
fs = 1/dt;
%%
ts.clear_var('roa_in_endfeet_transitions_strict');
...
state_previous_start = tbl_episodes.PreviousStateStart(I_ep);
...
roa_density_transitions = begonia.processing.create_transition_traces_strict(t,tbl_roa_endfeet.roa_density_trace',I_roi,state_start,state_previous_start,state_end)';
roa_frequency_transitions = begonia.processing.create_transition_traces_strict(t,tbl_roa_endfeet.roa_frequency_trace',I_roi,state_start,state_previous_start,state_end)';

roa_in_endfeet_transitions_strict = table(roi_id,roi_group,state,state_previous,state_duration,state_start,state_end,roa_density_transitions,roa_frequency_transitions);

ts.save_var(roa_in_endfeet_transitions_strict)
end
