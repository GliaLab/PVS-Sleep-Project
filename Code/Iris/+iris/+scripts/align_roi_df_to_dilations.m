...
for i = 1:length(ts)
    ...
    roi_df_time_series = iris.position.join_by_center_distance( ...
        roi_df_time_series, ...
        vessel_diameter(:,["linescan_id","center"]), ...
        distance_threshold_pix);
    
    % Align the time series to the dilation time points.
    roi_df_aligned_to_dilations = ...
        iris.time_series.align_time_series(vessel_dilations(:,["t0","t0_id","linescan_id"]), roi_df_time_series, 15, 15);
    roi_df_aligned_to_dilations.name(:) = "ROI df/f0 aligned to dilations";
    
    if isempty(roi_df_aligned_to_dilations)
        begonia.logging.log(1,"Window outside bounds.");
        continue;
    end
    
    ts(i).save_var(roi_df_aligned_to_dilations);
end