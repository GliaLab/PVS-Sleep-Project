...
for i = 1:length(ts)
    ...
    fs_original = channel_time_series.fs(1);

    % Resample data to the lowest sampling frequency 
    channel_time_series = iris.time_series.resample(channel_time_series,fs);
    
    % Create baseline and decay episodes.
    baseline = table;
    baseline.ep = "Baseline";
    baseline.ep_start = tbl.Start_baseline(I) / fs_original;
    baseline.ep_end = tbl.End_baseline(I) / fs_original;
    decay = table;
    decay.ep = "Decay";
    decay.ep_start = tbl.AmplitudeFrame(I) / fs_original;
    decay.ep_end = tbl.DecayEndFrame(I) / fs_original;
    ...
    % sampling frequency.
    st = tbl.AmplitudeFrame(I) / fs_original;
    st = round(st * fs) + 1;
    en = tbl.DecayEndFrame(I) / fs_original;
    en = round(en * fs) + 1;
    b_st = tbl.Start_baseline(I) / fs_original;
    b_st = round(b_st * fs) + 1;
    b_en = tbl.End_baseline(I) / fs_original;
