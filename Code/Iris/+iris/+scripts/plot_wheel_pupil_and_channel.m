...
ts = ts(ts.find_dnode("labview").has_var("pupil_diameter"));
%%
close all force
tic
for i = 1:length(ts)
    ...
    pupil_diameter = ts(i).find_dnode("labview").load_var("pupil_diameter");
    
    % Init figure.
    tiles = [height(average_images)+2,4];
    f = figure;
    ...
    nexttile();
    nexttile([1,3]);
    iris.time_series.plot_time_series(pupil_diameter);
    
    % Save
    trial_id = average_images.trial_id(1);
    filename = fullfile(get_project_path, "Plot", "Wheel, pupil and channel", trial_id);
