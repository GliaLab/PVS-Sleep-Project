function [estimates,p_values,model,r_ef] = estimate_with_random(varargin)
p = inputParser;
...
if isempty(predictor_2)
    ...
    for i = 1:length(categories_1)
        ...
        [~,~,z] = mdl.randomEffects;
        name = categorical(z.Name);
        I = name == '(Intercept)';
        
        r_ef = table;
        r_ef.group = z.Group(I);
        r_ef.level = z.Level(I);
        r_ef.name(:) = categories_1(1);
        r_ef.estimate = link_function(z.Estimate(I) + mdl.Coefficients.Estimate(1));
        r_ef.lwr = link_function(z.Estimate(I) - z.SEPred(I) + mdl.Coefficients.Estimate(1));
        r_ef.upr = link_function(z.Estimate(I) + z.SEPred(I) + mdl.Coefficients.Estimate(1));
        
        r_ef.group = categorical(r_ef.group);
        r_ef.level = categorical(r_ef.level);
        r_ef.name = categorical(r_ef.name);
