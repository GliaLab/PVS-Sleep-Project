...
scans = scans(scans.has_var('eeg_pvs'));
...
for i = 1:length(scans)
    ...
    eeg_pvs = scans(i).load_var('eeg_pvs');
    eeg_pvs_tbl = scans(i).load_var('eeg_pvs_tbl');
    ...
    eeg_pvs_tbl.color = turbo(height(eeg_pvs_tbl));
    
    % Assign values from the input data.
    slow_delta = eeg_pvs_tbl.slow_delta;
    t = eeg_pvs.t;
    f = eeg_pvs.f;
    ...
    spectrogram = cat(3,eeg_pvs_tbl.spectrogram{:});
    ...
    p = gobjects(0);
    for j = 1:height(eeg_pvs_tbl)
        p(j) = plot(t,slow_delta(j,:),"Color",eeg_pvs_tbl.color(j,:),"LineWidth",2);
    end
    ylabel("Power")
    title("Slow delta (0.2-4 Hz) ECoG power aligned to PVS peak")
    ...
    ax(4).YScale = 'log';
    ylabel('Frequency (Hz)');
    title(sprintf("Average ECoG Spectrogram aligned to PVS peak, N = %d",height(eeg_pvs_tbl)));
    xlabel("Time from dilation (seconds)")
    
    filename = fullfile(eustoma.get_plot_path,'Linescan PVS timepoint per trial ECoG',trial_id+".png");
