...
for i = 1:length(scans)
    ...
    for j = 1:N_vessels
        ...
        diameter = diameter_green.diameter{j};
        fs = diameter_green.vessel_fs(j);
        flims = [0.1,50];
        ...
        st = diameter_green.vessel_upper{j};
        en = diameter_green.vessel_lower{j};
        ...
        % Save
        filename = char(vessels_green.vessel_name(j));
        % Make folders that separate sleep/wake and vessel types.
        filename = fullfile(trial_type, vessel_type, filename);
        filename = fullfile(eustoma.get_plot_path,'Linescan Diameter Spectrograms Green',filename);
