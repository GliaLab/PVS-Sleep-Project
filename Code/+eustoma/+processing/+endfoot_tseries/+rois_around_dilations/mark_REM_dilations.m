...
actions(end+1) = xylobium.dledit.Action('Mark REM dilation', ...
    @(ts,~,~) rem_dilation(ts), ...
    false, false);

initial_vars = {};
initial_vars{end+1} = 'path';
initial_vars{end+1} = 'rem_dilation_eps';

xylobium.dledit.Editor(ts,actions,initial_vars,[],false); 
%%

function rem_dilation(ts)
...
yucca.processing.mark_episodes.MarkEpisodes(ts,'rem_dilation_eps',id,x,y,episode_choices,tbl);
end
