...
ts = ts(ts.has_var('roi_cap_dilation'));
...
roi_cap_dilation = ts.load_var('roi_cap_dilation');
roi_cap_dilation = cat(1,roi_cap_dilation{:});
...
[G,tbl] = findgroups(roi_cap_dilation(:,["vessel_id","ep_id"]));
tbl.signal = splitapply(@(x)mean(x,1), roi_cap_dilation.signal, G);
tbl.fs = splitapply(@(x)x(1), roi_cap_dilation.fs, G);
roi_cap_dilation = tbl;
%%
...
N_rois = height(roi_cap_dilation);
roi_avg = mean(roi_cap_dilation.signal,1);
roi_std = std(roi_cap_dilation.signal,[],1);
roi_confidence = 1.96 * roi_std / sqrt(N_rois);

roi_t = (0:length(roi_avg)-1) / roi_cap_dilation.fs(1); 
roi_t = roi_t - roi_t(round(length(roi_t)/2));

p = plot(roi_t,roi_avg);
yucca.plot.plot_around_line(p,roi_confidence)
title(sprintf('Glt1 vessel center ROI df/f0 (average +/- 1.96 sem), N = %.f',N_rois))
ylabel('df/f0')
...
filename = fullfile(eustoma.get_plot_path,"Endfeet Vessel diameter and center vessel ROI traces around dilation average", ...
