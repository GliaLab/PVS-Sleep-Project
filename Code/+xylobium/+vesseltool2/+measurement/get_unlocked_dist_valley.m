function [diameter_m, diameter_lrd, means, left_point, right_point] = get_unlocked_dist_valley(lscan)
    ...
    [diameter_m] = get_dist(lscan, means_min);
...
function [diameter, left_point, right_point] = get_dist(lscan, means)
    ...
    % interception:
    left_point = find(lscan(1:mid) > left_mean, 1, 'last');
    right_point = find(lscan(mid+1:end) > right_mean, 1, 'first') + mid;
